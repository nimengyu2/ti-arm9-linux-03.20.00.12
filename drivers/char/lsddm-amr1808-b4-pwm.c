...
// 这里使用的引脚是
//SPI0_ENA/EPWM0B/PRU0_R30[6]   EPWM0B的功能
...
// pwm初始化  设定低频的时候使用这个函数 这个函数中
// 设定pwm预分频128×14
int fn_pwm_init_slow(void)
{
	unsigned short u16_tmp;
	int i;
	unsigned short u16_period;
	clk_enable(&pwm1_clk);	
	...
	u16_period = (150000000/(128*14))/g_u32_pwm_output_freq;
	...
	for(i=0;i<7;i++)
	{
		u16_tmp = __raw_readw(EHRPWM0_TBCNT);	     
   	...
   	switch (cmd)
   	{
			...
			g_u32_pwm_output_freq = arg;
      		...
      		case SET_PWM_OFF:
       		fn_pwm_exit();
       		break;     
			case SET_PWM_SLOW_ON:
			// 低频的情况下 150M/128/14的主时钟下
			g_u32_pwm_output_freq = arg;
       		fn_pwm_init_slow();
       		break;   
       		default:
