 ...
 */
void suart_timer2_config( void )
{
       unsigned int u32value = 0;

       // Reset T12 / T34
       __raw_writel(0x00000000, IO_ADDRESS(TIMER2BASE + TGCR));
       // Clear T12 counter
       __raw_writel(0x00000000, IO_ADDRESS(TIMER2BASE + TIM12));
       // Disable T12 / T34 counters
       __raw_writel(0x00000000, IO_ADDRESS(TIMER2BASE + TCR));
       // Set T12 period
       __raw_writel(0xFFFFFFF0, IO_ADDRESS(TIMER2BASE + PRD12));
       // Clear COMP registers
       __raw_writel(0xFFFFFFFF, IO_ADDRESS(TIMER2BASE + CMP0));
       __raw_writel(0xFFFFFFFF, IO_ADDRESS(TIMER2BASE + CMP1));
       __raw_writel(0xFFFFFFFF, IO_ADDRESS(TIMER2BASE + CMP2));
       __raw_writel(0xFFFFFFFF, IO_ADDRESS(TIMER2BASE + CMP3));
       __raw_writel(0xFFFFFFFF, IO_ADDRESS(TIMER2BASE + CMP4));
       __raw_writel(0xFFFFFFFF, IO_ADDRESS(TIMER2BASE + CMP5));
       __raw_writel(0xFFFFFFFF, IO_ADDRESS(TIMER2BASE + CMP6));
       __raw_writel(0xFFFFFFFF, IO_ADDRESS(TIMER2BASE + CMP7));
       // Enable and initalize T12
       __raw_writel(0x00000015, IO_ADDRESS(TIMER2BASE + TGCR));
       // Clear and enable T12 interrupt
       __raw_writel(0x000A000A, IO_ADDRESS(TIMER2BASE + INTCTLSTAT));
       u32value = __raw_readl(IO_ADDRESS(TIMER2BASE + INTCTLSTAT));
       __raw_writel((u32value | 0x01), IO_ADDRESS(TIMER2BASE + INTCTLSTAT));
       // Enable T12 counter
       __raw_writel(0x00000080, IO_ADDRESS(TIMER2BASE + TCR));
}
...
/*
 * ehrpwm psc enable routine 
 */
void scrdr_ehrpwm_psc_enable(unsigned int psc1_addr)
{
	CSL_PscRegsOvly PSC = (CSL_PscRegsOvly) psc1_addr;	// CSL_PSC_1_REGS;
	...
	// If we are already in that state, just return
	if (CSL_FEXT(PSC->MDSTAT[CSL_PSC_EHRPWM], PSC_MDSTAT_STATE) ==
	    CSL_PSC_MDSTAT_STATE_ENABLE)
		return;

	// Perform transition
	CSL_FINST(PSC->MDCTL[CSL_PSC_EHRPWM], PSC_MDCTL_NEXT, ENABLE);
	CSL_FINST(PSC->PTCMD, PSC_PTCMD_GO0, SET);
	...
	// Wait and verify the state
	while (CSL_FEXT(PSC->MDSTAT[CSL_PSC_EHRPWM], PSC_MDSTAT_STATE) !=
	       CSL_PSC_MDSTAT_STATE_ENABLE) ;
}

/*
 * ehrpwm psc disable routine 
 */
void scrdr_ehrpwm_psc_disable(unsigned int psc1_addr)
{
	CSL_PscRegsOvly PSC = (CSL_PscRegsOvly) psc1_addr;	// CSL_PSC_1_REGS;
	...
	// Perform transition
	CSL_FINST(PSC->MDCTL[CSL_PSC_EHRPWM], PSC_MDCTL_NEXT, SYNCRST);
	CSL_FINST(PSC->PTCMD, PSC_PTCMD_GO0, SET);
