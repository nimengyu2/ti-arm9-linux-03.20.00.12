...
short pru_softuart_setdir
	...
	unsigned int temp;
       ...
       if (regval > 0) {
		...
		// Regardless of DIR mode, store DIR content to PRU0
		offset =
                     PRU_SUART_PRU0_CH0_OFFSET + ((hUart->uartNum - 1) * 4) + PRU_SUART_CH_DIRDEL_OFFSET;
	...
	if (configUart->TXSerializer != PRU_SUART_SERIALIZER_NONE) {
		...
		*/
		...
		/* Configuring TX DIR values */
			...
			// Regardless of PRU mode, DIR data is saved in PRU0 DRAM
			offset =
                            PRU_SUART_PRU0_CH0_OFFSET + ((hUart->uartNum - 1) * 4) + PRU_SUART_CH_DIRPIN_OFFSET;
