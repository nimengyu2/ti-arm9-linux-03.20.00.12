...
#define EHRPWM_PHYS_ADDRESS  0x01F02000u
...
struct omapl_pru_suart {
	...
	const struct firmware *fw_pru0;
	const struct firmware *fw_pru1;
	...
	u32 clk_freq_timer2;
	u32 tx_loadsz;
    char ifd_cmd[NR_SUART];
    char ifd_rsp[NR_SUART];
};

// TBD need to move these to proper header file
/* IFD Commands */
...
#define IFD_CMD_RSP_ICCPRESENT   0x43
#define IFD_CMD_RSP_ICCABSENT    0x44


static ssize_t show_reader_cmd (struct device *dev, struct device_attribute *attr, char *buf)
{
	int reader_num = attr->attr.name[9] - 0x30; 
	struct omapl_pru_suart *soft_uart = (struct omapl_pru_suart *) dev_get_drvdata(dev);

//    printk(KERN_ERR "%s reader:%d cmd:0x%x status:0x%x\n", __FUNCTION__, reader_num, soft_uart->ifd_cmd[reader_num], soft_uart->ifd_rsp[reader_num]);
	return (snprintf(buf, PAGE_SIZE, "%c\n", soft_uart->ifd_rsp[reader_num]));
}

static ssize_t exec_reader_cmd (struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int reader_num = attr->attr.name[9] - 0x30; 
	struct omapl_pru_suart *soft_uart = (struct omapl_pru_suart *) dev_get_drvdata(dev);

	soft_uart->ifd_cmd[reader_num] = buf[0];
	soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_INPROGRESS;

	switch(soft_uart->ifd_cmd[reader_num]) {
		case IFD_CMD_ICC_PRESENT:
			if(pru_scrdr_iscard_present(&soft_uart->pru_arm_iomap) == PRU_SUART_SUCCESS)
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_SUCCESS;
			else
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_FAIL;
			break;
		case IFD_CMD_ICC_POWERUP:
			if(pru_scrdr_iscard_present(&soft_uart->pru_arm_iomap) == PRU_SUART_SUCCESS) {
				pru_scrdr_powerup_card(&soft_uart->pru_arm_iomap);
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_SUCCESS;
			}
			else
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_FAIL;
			break;
		case IFD_CMD_ICC_POWERDOWN:
			if(pru_scrdr_iscard_present(&soft_uart->pru_arm_iomap) == PRU_SUART_SUCCESS) {
				pru_scrdr_powerdown_card(&soft_uart->pru_arm_iomap);
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_SUCCESS;
			}
			else
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_FAIL;
			break;
		case IFD_CMD_ICC_POWERRESET:
			if(pru_scrdr_iscard_present(&soft_uart->pru_arm_iomap) == PRU_SUART_SUCCESS) {
				pru_scrdr_reset_card(&soft_uart->pru_arm_iomap);
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_SUCCESS;
			}
			else
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_FAIL;
			break;
		case IFD_CMD_ICC_ENABLEVCC:
			if(pru_scrdr_iscard_present(&soft_uart->pru_arm_iomap) == PRU_SUART_SUCCESS) {
				pru_scrdr_enable_vcc(&soft_uart->pru_arm_iomap);
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_SUCCESS;
			}
			else
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_FAIL;
			break;
		case IFD_CMD_ICC_ENABLERESET:
			if(pru_scrdr_iscard_present(&soft_uart->pru_arm_iomap) == PRU_SUART_SUCCESS) {
				pru_scrdr_enable_reset(&soft_uart->pru_arm_iomap);
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_SUCCESS;
			}
			else
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_FAIL;
			break;
		case IFD_CMD_ICC_ISVCCENABLED:
			if( pru_scrdr_isvccenabled(&soft_uart->pru_arm_iomap) == PRU_SUART_SUCCESS)
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_SUCCESS;
			else
				soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_FAIL;
			break;
		default:
			soft_uart->ifd_rsp[reader_num] = IFD_CMD_RSP_NOTSUPPORTED;
			break;
	}
...
/* Number of Device Attributes depends on the number of
   Card Readers Supported */
static DEVICE_ATTR(sc_reader0_cmd, 0644, show_reader_cmd, exec_reader_cmd);

static void sc_reader_create_sysfs (struct device *dev)
{
	int status;
	status = device_create_file(dev, &dev_attr_sc_reader0_cmd);
}

static void __devexit sc_reader_remove_sysfs (struct device *dev)
{
	device_remove_file(dev, &dev_attr_sc_reader0_cmd);
}
...
static int __devinit omapl_pru_suart_probe(struct platform_device *pdev)
{
    ...
    /* NOTE: Should probably add additional iounmap tags here for these
             error cases
    */
    soft_uart->pru_arm_iomap.gpio_io_addr =
        IO_ADDRESS(DA8XX_GPIO_BASE);
    if (!soft_uart->pru_arm_iomap.gpio_io_addr) {
        dev_err(&pdev->dev, "ioremap failed\n");
        err = -ENOMEM;
        goto probe_exit_iounmap_4;
    }

    soft_uart->pru_arm_iomap.ehrpwm1_io_addr = IO_ADDRESS(EHRPWM_PHYS_ADDRESS);
    if (!soft_uart->pru_arm_iomap.ehrpwm1_io_addr) {
        dev_err(&pdev->dev, "ioremap failed\n");
        err = -ENOMEM;
        goto probe_exit_iounmap_4;
    }
    ...
    /* Request the firmware for PRU0 */
	err = request_firmware(&soft_uart->fw_pru0, "PRU_SUART_SC_Emulation.bin",
			       &pdev->dev);
	if (err) {
	...
	dev_info(&pdev->dev, "fw_pru0 size %td. downloading...\n",
		 soft_uart->fw_pru0->size);

    /* Request the firmware for PRU1 */
	err = request_firmware(&soft_uart->fw_pru1, "PRU_SCARD_Emulation.bin",
			       &pdev->dev);
	if (err) {
		dev_err(&pdev->dev, "can't load firmware\n");
		err = -ENODEV;
	...
	dev_info(&pdev->dev, "fw_pru1 size %td. downloading...\n",
		 soft_uart->fw_pru1->size);
    ...
    /* We can probably make the firmware an array and just pass the arrray */
	err = pru_softuart_init(SUART_DEFAULT_BAUD, SUART_DEFAULT_BAUD,
			      SUART_DEFAULT_OVRSMPL, soft_uart->fw_pru0->data,
			      soft_uart->fw_pru0->size, soft_uart->fw_pru1->data,
                  soft_uart->fw_pru1->size, &soft_uart->pru_arm_iomap);
	if (err) {
    ...
    pru_screader_init(&soft_uart->pru_arm_iomap);
    ...
    sc_reader_create_sysfs(&pdev->dev);
...
probe_release_fw:
	release_firmware(soft_uart->fw_pru0);
	release_firmware(soft_uart->fw_pru1);
probe_exit_clk:
...
static int __devexit omapl_pru_suart_remove(struct platform_device *pdev)
{
    ...
    sc_reader_remove_sysfs(&pdev->dev);
